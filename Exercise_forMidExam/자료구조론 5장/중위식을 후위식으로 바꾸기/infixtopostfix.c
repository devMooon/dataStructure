...
int main()
{
	strcpy(expr, "A*(B-C)*D/E "); // 입력의 마지막에 공백(eos)을 둔다.
	postfix();
	strcpy(expr, "A-B*D+E/(F+G)-H "); // 입력의 마지막에 공백(eos)을 둔다.
	postfix();
	strcpy(expr, "(1-2)*7 "); // 입력의 마지막에 공백(eos)을 둔다.
	postfix();
	strcpy(expr, "A/B-C+D*E-A*C "); // 입력의 마지막에 공백(eos)을 둔다.
	postfix();
}
