...
int garbage(int a[], int n, int min, int max, int reNum);
...
int main(void)
{
	...
	printf("평균 : %g\n", aver);
	printf("분산 : %g\n", variance(data, data2, size, aver)); // 분산 = 모두 더해(편차*편차)/10 , 편차 제곱의 평균
...
int garbage(int a[], int n, int min, int max, int reNum) //6)을 위해 min, max 값 추가
{
	int garb = 0;
	for (int i = 0; i < n; i++)
	{
